...
| Video Generation | `POST /v1/videos/generations` | Local OpenAI-shaped image-to-video surface for the configured Wan2.2 TI2V 5B Turbo Q3 asset using a native Diffusers runner. Downloads one GGUF transformer plus required Diffusers-side tokenizer, text encoder, scheduler, VAE, and configs. |
